...
return (
    ...
    >
        ...
        {isOpen && children && (
            <ul className={`absolute top-8 ${alignRight ? 'right-0' : 'left-0'} mt-1 bg-white/90 shadow-lg rounded-md py-1 min-w-[200px] z-10`}>
                {children}
            </ul>
        )}
    </li>
  ...
  return (
    ...
    >
      ...
      {isOpen && subsections && subsections.length > 0 && (
        <ul className={`text-sm absolute ${alignLeft ? 'right-full mr-[1px]' : 'left-full ml-[1px]'} top-0 bg-white/90 shadow-lg rounded-md py-1 min-w-[200px] z-20`}>
...
return (
  <>
    <nav className="sticky top-0 z-50 shadow-lg bg-bs-primary">
        <ul className="flex flex-wrap items-center justify-center gap-1 px-4 py-3">
            <li>
                    <Link 
                      to="/" 
                      onMouseDown={() => {
    ...
    {isModalOpen && modalContent && (
      <SecurityModal 
        ...
        security={parseMarkup(modalContent.security)}
